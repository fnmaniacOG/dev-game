...
declare_id!("8P9Dzoa4EYPxguBBMjbiqjV76NBkCm7JUG7AEh1hCtop");
...
pub mod tournament {
    ...
    pub fn create_tournament(
        ...
        let nb = params.name.as_bytes();
        let nc = nb.len().min(64);
        t.name[..nc].copy_from_slice(&nb[..nc]);
        let num = params.prize_shares.len().min(10);
        t.prize_shares[..num].copy_from_slice(&params.prize_shares[..num]);
        t.num_prize_tiers  = num as u8;
    ...
    pub fn enter_tournament(ctx: Context<EnterTournament>) -> Result<()> {
        ...
        if entry_fee > 0 {
            ...
            ctx.accounts.tournament.prize_pool = ctx.accounts.tournament
                .prize_pool
	...
	require!(rank > 0 && rank <= t.num_prize_tiers as usize, TourneyError::NoPrize);
...
pub struct CreateTournament<'info> {
    ...
    #[account(
        ...
        seeds = [b"tournament", creator.key().as_ref(), game.key().as_ref(), mint.key().as_ref()],
        bump,
    )]
